...
# 5) CUDA 渲染（只渲 3D）
sv0   = sv[0].to(DEVICE)
l3_0  = l3_out[0].to(DEVICE)
render = Render(device='cuda' if torch.cuda.is_available() else 'cpu')
out_path = render.rendering_3d_only(
    OUT_DIR,
    "sample0",
    l3_0,
)
print("3D-only render saved at", out_path)
