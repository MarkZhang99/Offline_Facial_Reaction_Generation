...
from model.BasicBlock import ConvBlock, PositionalEncoding, lengths_to_mask, init_biased_mask
...
class VAEModel(nn.Module):
    ...
    def forward(self, x):
        # x: (B, T, in_channels)
        x = self.linear(x)      # B, T, latent_dim
        B, T, _ = x.shape
        lengths = [T] * B       # we always pad/truncate to fixed T

        mu_token     = self.mu_token.unsqueeze(0).expand(B, -1).unsqueeze(1)      # B,1,D
        logvar_token = self.logvar_token.unsqueeze(0).expand(B, -1).unsqueeze(1)  # B,1,D

        x = torch.cat([mu_token, logvar_token, x], dim=1)  # B, T+2, D
        x = x.transpose(0,1)                              # T+2, B, D
        dev = x.device
        token_mask = torch.ones(B, 2, dtype=torch.bool, device=dev)
        mask       = lengths_to_mask(lengths, dev)

        aug_mask = torch.cat([token_mask, mask], dim=1)    # B, T+2
        ...
        motion_sample = self.sample_from_distribution(dist).to(dev)
...
class Decoder(nn.Module):
    ...
    def forward(self, encoded_feature, past_reaction_3dmm=None, past_reaction_emotion=None):
        # encoded_feature: (B, T, D)
        B, TS, _ = encoded_feature.shape
        TL = self.window_size if self.online else TS
        dev = encoded_feature.device
        motion_sample, dist = self.vae_model(encoded_feature)  # (B,T,D), list dist
        # prepare decoder queries & mask
        time_queries = torch.zeros(B, TL, self.feature_dim, device=dev)
        time_queries = self.PE(time_queries)
        # tgt_mask = self.biased_mask[:TL, :TL].to(dev).unsqueeze(0).expand(B, -1, -1)
        # pick a single 2-D mask, then broadcast to batch
        # replace with this
        tgt_mask = self.biased_mask[0, :TL, :TL].to(dev)  # just [TL,TL]


        listener_reaction = self.listener_reaction_decoder_1(
            tgt=time_queries,
            memory=motion_sample.unsqueeze(1),
            tgt_mask=tgt_mask
        )
...
class TransformerVAE(nn.Module):
    ...
    def forward(self, speaker_video=None, speaker_audio=None, **kwargs):
        ...
        if self.online:
            ...
            dev = input.device
            reaction_3dmm = torch.zeros((speaker_video.size(0), self.window_size, self.output_3dmm_dim), device = dev)
            reaction_emotion = torch.zeros((speaker_video.size(0), self.window_size, self.output_emotion_dim), device = dev)
