...
class Decoder(nn.Module):
    ...
    def forward(self, encoded_feature, past_reaction_3dmm = None, past_reaction_emotion = None):
        ...
        tgt_mask = self.biased_mask[:, :TL, :TL].clone().detach().to(dev).repeat(B,1,1)
        ...
        if self.online and (past_reaction_3dmm is not None):
            ...
            tgt_mask = self.biased_mask[:, :(TL + past_reaction_3dmm.shape[1]), :(TL + past_reaction_3dmm.shape[1])].detach().to(dev).repeat(B,1,1)
