...
def test():
    sys.stdout = Logger(osp.join(args.logs_dir, 'val.txt'))
    print("==========\nArgs:{}\n==========".format(args))
    ...
    testset = ActionData(root=args.data_dir, data_type='val', neighbors=None,
                         neighbor_pattern=args.neighbor_pattern, num_frames=num_frames, stride=stride)
    ...
    val_path = pd.read_csv(os.path.join(args.data_dir, 'val.csv'), header=None, delimiter=',')
    ...
    val_neighbour_path = os.path.join(args.data_dir, 'neighbour_emotion_val.npy')
