...
def train(args, model, train_loader, optimizer, criterion):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        listener_emotion,
        listener_3dmm,
        listener_reference) = batch

        
        optimizer.zero_grad()
        listener_3dmm_out, listener_emotion_out, dist = model(speaker_video_clip,
                                                            speaker_audio_clip)
...
def main(args):
    ...
    tb_logdir = os.path.join(args.outdir, "runs")
    writer = SummaryWriter(log_dir=tb_logdir)


    for epoch in range(start_epoch, args.epochs):
        train_loss, rec_loss, kld_loss, div_loss = train(args, model, train_loader, optimizer, criterion)
        # 2) write in TensorBoard (Train metrics)
        writer.add_scalar("Loss/train_total", train_loss, epoch)
        writer.add_scalar("Loss/train_rec",   rec_loss,   epoch)
        writer.add_scalar("Loss/train_kld",   kld_loss,   epoch)
        writer.add_scalar("Loss/train_div",   div_loss,   epoch)
        ...
        if (epoch+1) % 1 == 0:
            ...
            # 3) write TensorBoard(val metrics)
            writer.add_scalar("Loss/val_total", val_loss, epoch)
            writer.add_scalar("Loss/val_rec", rec_loss, epoch)
            writer.add_scalar("Loss/val_kld", kld_loss, epoch)
