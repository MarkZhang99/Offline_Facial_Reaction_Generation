...
from trainer.dataset import get_dataloader
...
def parse_arg():
    ...
    parser.add_argument('-j', '--num_workers', default=2, type=int, metavar='N',
    ...
    parser.add_argument('--no-render', action='store_true',
                    help="skip any video/mesh rendering, only compute metrics")
    ...
    for batch_idx, (speaker_video_clip, speaker_audio_clip, speaker_emotion, _, listener_video_clip, _, listener_emotion, listener_3dmm, listener_references) in enumerate(tqdm(val_loader)):
        ...
        with torch.no_grad():
            ...
            if isinstance(prediction, (list, tuple)):
                ...
                loss, rec_loss, kld_loss = criterion(
                    listener_emotion,
                    listener_3dmm,
                ...
                # BeLFusion 的 loss 函数返回一个 dict，里面有 "loss" 键
                loss = criterion(**prediction)["loss"].item()
                losses.update(loss, speaker_video_clip.size(0))

            else:
                raise RuntimeError(f"Unexpected prediction type: {type(prediction)}")
            ...
            if (batch_idx % 25) == 0:
                for bs in range(B):
                    if do_render and render is not None:
        ...
        speaker_video_clip, speaker_audio_clip, speaker_emotion, _, _, _, listener_emotion, _, _) in enumerate(tqdm(val_loader)):
            ...
            with torch.no_grad():
                ...
                if isinstance(prediction, (tuple, list)):
                    _, listener_emotion_out, _ = prediction
                elif isinstance(prediction, dict):
                    listener_emotion_out = prediction["prediction"]
                else:
                    raise RuntimeError(f"Unexpected prediction type: {type(prediction)}")
...
def main(args):
    ...
    if args.no_render:
        ...
        render = Render(device.type)   # or 'cpu'/'cuda' as before
